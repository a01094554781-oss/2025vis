...
try:
    ...
    if region_col in df.columns:
        # 영어 지역명 컬럼 추가
        df['region_en'] = df[region_col].map(region_map).fillna(df[region_col])
        ...
        st.error("Error: Region column not found.")
        st.stop()
...
txt = UI_TEXT[lang] # 선택된 언어의 텍스트 팩 가져오기
...
# 필터 2: 지역 (언어에 따라 표시 다르게)
r_col = 'region_en' if lang == 'en' else region_col
    ...
    with col2:
        ...
        if not filtered_df.empty:
            ...
            st.dataframe(
                ...
                column_config={'축제명': txt['col_name'], r_col: txt['col_region']}
            )
...
with tab4:
    ...
    # 언어 바뀔 때 환영 메시지 리셋 (선택사항, 여기서는 유지)
